...
class Zip {
    ...
    public function finalize() {
        ...
        $log = new \com\grandt\Logger("PHPZip Log", true);
        $log->logLine("isFinalized? " . ($this->isFinalized ? "TRUE" : "FALSE"));

        if (!$this->isFinalized) {
            $log->logLine("streamFilePath: " . $this->streamFilePath);
            if (strlen($this->streamFilePath) > 0) {
                $this->closeStream();
            }
            $log->logLine("Central Directory #entries: " . sizeof($this->cdRec));

            $cd = implode("", $this->cdRec);

            $log->logLine("Central Directory length..: " . strlen($cd));
            $log->logLine("Packing Central Directory");

            $cdRecSize = pack("v", sizeof($this->cdRec));

            $cdRec = $cd . self::ZIP_END_OF_CENTRAL_DIRECTORY . $cdRecSize . $cdRecSize . pack("VV", strlen($cd), $this->offset);

            $log->logLine("Adding Zip Comment");
            ...
            $log->logLine("Write CD");

            $this->zipwrite($cdRec);

            $log->logLine("Mark finalized, and clear CD");
    ...
    private function getDosTime($timestamp = 0) {
        ...
        if ($date["year"] >= 1980) {
            return pack("V", (($date["mday"] + ($date["mon"] << 5) + (($date["year"] - 1980) << 9)) << 16) | (($date["seconds"] >> 1) + ($date["minutes"] << 5) + ($date["hours"] << 11)));
        }
    ...
    private function buildZipEntry($filePath, $fileComment, $gpFlags, $gzType, $timestamp, $fileCRC32, $gzLength, $dataLength, $extFileAttr) {
        ...
        if ($isFileUTF8 || $isCommentUTF8) {
            ...
            if ($isFileUTF8) {
                $utfPathExtraField = "\x75\x70" . pack("v", (5 + strlen($filePath))) . "\x01" . pack("V", crc32($filePath)) . $filePath;
            ...
            if ($isCommentUTF8) {
                $centralExtraField .= "\x75\x63" // utf8 encoded file comment extra field
                    . pack("v", (5 + strlen($fileComment))) . "\x01" . pack("V", crc32($fileComment)) . $fileComment;
            }
        }

        $header = $gpFlags . $gzType . $dosTime . $fileCRC32 . pack("VVv", $gzLength, $dataLength, strlen($filePath)); // File name length

        $zipEntry = self::ZIP_LOCAL_FILE_HEADER . self::ATTR_VERSION_TO_EXTRACT . $header . pack("v", strlen($localExtraField)) // Extra field length
            . $filePath // FileName
            . $localExtraField; // Extra fields

        $this->zipwrite($zipEntry);

        $cdEntry = self::ZIP_CENTRAL_FILE_HEADER . self::ATTR_MADE_BY_VERSION . ($dataLength === 0 ? "\x0A\x00" : self::ATTR_VERSION_TO_EXTRACT) . $header . pack("v", strlen($centralExtraField)) // Extra field length
