 ...
 *
 * Modified from GIF Animation Resizer by Taha Paksu.
 * http://www.phpclasses.org/package/7353-PHP-Resize-animations-in-files-of-the-GIF-format.html
 * Original released under the MIT/X Consortium License.
 *
...
class gifresizer {
     ...
     */
    function resize($orgfile,$newfile,$width=0,$height=0){
        $this->decode($orgfile);
        if ($width == 0 && $height == 0) {
            $this->wr = 1;
            $this->hr = 1;
            ...
            if ($width > 0) {
                ...
                if ($height > 0) {
                    ...
                    $this->hr=(((int)ceil($this->wr*$this->originalheight))/$this->originalheight);
                    $height = $this->hr*$this->originalheight;
                }
            } else {
                $this->hr=$height/$this->originalheight;
                $this->wr=(((int)ceil($this->hr*$this->originalwidth))/$this->originalwidth);
                $width = $this->wr*$this->originalwidth;
            }
        }
        $this->resizeframes();
